...
function SmallCard({ item }: { item: NewsItem }) {
  return (
    <Link href={`/newsroom/${item.slug}`} data-news-card className="group flex flex-col gap-3 border-b border-neutral-200 pb-4 sm:border-b-0 sm:pb-0">
      {/* Image */}
...
function FeaturedCard({ item }: { item: NewsItem }) {
  return (
    <Link href={`/newsroom/${item.slug}`} className="group flex flex-col gap-3 border-b border-neutral-200 pb-4 sm:border-b-0 sm:pb-0">
      {/* Image */}
      ...
      <div className="flex flex-col items-start gap-2">
        <CardMeta date={item.date} category={item.category} />
        <h3 className="max-w-[25.5625rem] text-left font-serif text-[1.2rem] sm:text-[1.5rem] leading-[2rem] sm:leading-[2rem] font-normal text-neutral-800">
          {item.title}
        </h3>
      </div>
    </Link>
...
export default function NewsGrid() {
  ...
  return (
    <section className="flex w-full flex-col gap-10 sm:gap-6 lg:gap-[2rem] px-4 sm:px-6 lg:px-[4.167rem] py-6 sm:py-8 lg:py-[3rem]">
      {/* Featured row — large card left, small card right */}
      <div className="grid grid-cols-1 sm:grid-cols-[2fr_1fr] gap-10 sm:gap-4">
        <FeaturedCard item={featured} />
        <SmallCard item={sideCard} />
      </div>

      {/* Small-card rows */}
      <div ref={newCardsRef} className="flex flex-col gap-10 sm:gap-4">
        {visibleRows.map((row, ri) => (
          <div key={`row-${ri}`} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 sm:gap-4">
            {row.map((item, i) => (
